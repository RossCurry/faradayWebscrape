 ...
 * @param albums
 */
function getBatches(albums) {
    const batches = [];
    function recurse(albums) {
        if (!albums.length)
            return;
 ...
 * @param ctx
 */
export default async function getAlbumInfo(ctx, next) {
    const faradayAlbums = ctx.state.data.faraday;
    ...
    const albumInfo = await Promise.all(faradayAlbums.slice(0, 5).map(async (album) => {
...
async function searchSingleAlbum(album, authString) {
    if (album.isSoldOut || !album.title)
        return;
