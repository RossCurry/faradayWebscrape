 ...
 */
faradayRouter.get("/api/faraday/albums/batch", async (ctx, _next) => {
    const { mongo } = ctx.services;
    if (!mongo)
        throw new Error('No mongo object found');
    ...
    try {
        const spotifyData = await mongo.spotify?.getSpotifyAlbumData(null, Number(limit), Number(offset), filterParsed);
